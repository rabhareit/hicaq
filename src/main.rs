...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let cmd = app_from_crate!()
        .arg(Arg::from_usage(
            "<COMMAND> -c --command <COMMAND> 'Previous command name'",
        ))
        .arg(Arg::from_usage(
            "<STATUS> -s --status <STATUS> 'Success(0) or fail(else)'",
        ))
        .arg(Arg::from_usage(
            "<EXECUTED> -a --executed-at <EXECUTED> 'Executed at'",
        ))
        .arg(Arg::from_usage(
            "<ELAPSED> -e --elapsed <ELAPSED> 'Elapsed time'",
        ))
        ...
        Some(val) => {
            ...
            match seconds > 60 {
                true => {
                    ...
                    match minutes > 60 {
                        true => {
                            format!("{}h {}min {}sec", minutes / 60, minutes % 60, seconds % 60)
                        }
    ...
    let notif_title = if notif_status == "0" {
        "Command succeeded :ok_woman:"
    } else {
        "Command failed :no_good:"
    };
    let notif_color = if notif_status == "0" {
        "good"
    } else {
        "danger"
    };
    ...
    let command_field = AttachmentField {
        title: "Command".to_string(),
        value: format!("`{}`", prev_cmd),
        short: false,
    };
    let directory_filed = AttachmentField {
        title: "Directory".to_string(),
        value: format!("`{}`", current_dir.display()),
        short: false,
    };
    let hostname_field = AttachmentField {
        title: "Hostname".to_string(),
        value: whoami::hostname(),
        short: true,
    };
    let user_field = AttachmentField {
        title: "User".to_string(),
        value: whoami::username(),
        short: true,
    };
    let exec_field = AttachmentField {
        title: "executed at".to_string(),
        value: exec_at,
        short: true,
    };
    let elapsed_field = AttachmentField {
        title: "Elapsed time".to_string(),
        value: elapsed,
        short: true,
    };
